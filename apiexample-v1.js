...
function createTVShow(tvshowJSON) {
	...
	try {
		...
		elemImage.src = "images/no-img-portrait-text.png";
		elemImage.alt = "IMAGE_OF_SHOW"
	}
...
function showEpisodes(data, elemSummaryEpisode) {
	...
	var eps_number;
	var eps_summary = "n/a";
	var eps_img_url = "images/no-img-portrait-text.png";
	...
	for (episode in data) {
		...
		try { eps_img_url = data[episode].image.medium; } catch (error) { eps_img_url = "images/no-img-portrait-text.png" }
		...
		for (i in data){
			...
			try { 
				...
				if (data[i].person.image.medium === null ){
					elemImageCastPerson.src = "images/no-img-portrait-text.png" ;
				} else {
					elemImageCastPerson.src = data[i].person.image.medium;
				}
				
			} catch (error) { elemImageCastPerson.src = "images/no-img-portrait-text.png" }
...
async function createGallery (data){
	...
	for (i in gallery){
		...
		} catch (error) { elemImageGallery.src = "images/no-img-portrait-text.png" }
